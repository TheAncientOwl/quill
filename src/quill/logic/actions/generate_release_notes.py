    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        previous_tag = (
            subprocess.check_output(
                ["git", "describe", "--tags", "--abbrev=0", f"{last_tag}^"]
            )
            .decode("utf-8")
            .strip()
        )
        ...
        git_log_command = [
            "git",
            "log",
            "--pretty=format:* %C(auto)%h - %C(bold blue)[%an](https://github.com/%an)%C(reset)%C(auto)%d %C(white)%s%C(reset)",
            "--no-decorate",
            "--abbrev-commit",
            f"{previous_tag}..{last_tag}",
        ]
