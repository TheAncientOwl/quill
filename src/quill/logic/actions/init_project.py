...
class InitProject(argparse._StoreTrueAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        project_name = input(
            "Enter the Project Name (no spaces): ").strip()
        ...
        QUILL_PROJECT_TEMPLATES = os.path.join(
            QUILL_HOME, "project", "templates")
        ...
        shutil.move(os.path.join(os.getcwd(), "src", "main", "java",
                    "TOOLKIT_PACKAGE"), os.path.join(os.getcwd(), "src", "main", "java", group_id.replace(".", os.sep)))
        shutil.move(os.path.join(os.getcwd(), "src", "test", "java",
                    "TOOLKIT_PACKAGE"), os.path.join(os.getcwd(), "src", "test", "java", group_id.replace(".", os.sep)))

        # 2.3. Setup placeholders
        for root, dirs, files in os.walk(os.getcwd()):
            for file in files:
                if file.endswith(".java") or file.endswith(".xml") or file.endswith(".yml"):
        ...
        shutil.move(os.path.join(entry_point_path, "$TOOLKIT_PROJECT_NAME.java"),
                    os.path.join(entry_point_path, f"{project_name}.java"))

        # 3. Add quill.json
        local_quill_json = os.path.join(os.getcwd(), "quill.json")
        shutil.copy2(os.path.join(QUILL_PROJECT_TEMPLATES, "configs",
                     "feather-toolkit-config.linux.json"), local_quill_json)
